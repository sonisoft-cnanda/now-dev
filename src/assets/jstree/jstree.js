 ...
 */
	...
	$.jstree.plugins.checkbox = function (options, parent) {
		...
		this.activate_node = function (obj, e) {
            var isSelectiveSyncView = (document.title === 'Selective Sync');
            var currentTreeNode = $(e.target).closest('.jstree-node');

            // Selection on non leaf nodes is allowed in selective sync view. But in Diff view it is not allowed.
            if (!isSelectiveSyncView && currentTreeNode && !currentTreeNode.hasClass('jstree-leaf')) {
				return;
			}
	...
	$.jstree.plugins.contextmenu = function (options, parent) {
		...
		this.show_contextmenu = function (obj, x, y, e) {
			...
			if(s.show_at_node || x === undefined || y === undefined) {
				...
				// Fix to context menu display position issue
				y = o.top + a[0].clientHeight;
			}
			// the following code has been commented to prevent the selection of node when right click happens for context menu
 ...
 */
	...
	(function ($, undefined) {
		$.vakata.storage = {
			...
			set : function (key, val) { return null; /*window.localStorage.setItem(key, val)*/ },
			get : function (key) { return null; /*window.localStorage.getItem(key);*/ },
			del : function (key) { return null; /*window.localStorage.removeItem(key);*/ }
		};
	}($));
